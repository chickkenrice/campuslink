...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_assignment'])) {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            $msg = "Assignment created successfully for $targetGroup!"; 
            $msgType = "success";
        } else {
...
<head>
    ...
    <style>
        ...
        /* Month/Year Header */
        .flatpickr-months { padding: 10px 0; background: transparent; }
        .flatpickr-months .flatpickr-month { color: white; fill: white; height: 40px; background: transparent; }
        .flatpickr-current-month { font-size: 16px; font-weight: 600; color: white; }
        ...
        .flatpickr-current-month .flatpickr-monthDropdown-months:hover { background: #3f3f5a !important; }
        .flatpickr-current-month input.cur-year { 
            color: white !important; 
            font-weight: 600; 
            background: transparent !important;
        }
        .flatpickr-months .flatpickr-prev-month, .flatpickr-months .flatpickr-next-month { fill: white; color: white; padding: 8px; }
        .flatpickr-months .flatpickr-prev-month:hover, .flatpickr-months .flatpickr-next-month:hover { background: rgba(255,255,255,0.1); border-radius: 50%; }
        .flatpickr-months .flatpickr-prev-month svg, .flatpickr-months .flatpickr-next-month svg { fill: white !important; }
        
        /* Weekday Headers */
        .flatpickr-weekdays { background: transparent; }
        .flatpickr-weekday { color: #9ca3af !important; font-size: 12px; font-weight: 600; background: transparent !important; }
        
        /* Days Grid */
        .flatpickr-days { background: transparent !important; overflow: visible !important; }
        ...
        .flatpickr-day:hover { 
            background: rgba(128, 86, 255, 0.3) !important; 
            border-color: transparent !important; 
            color: white !important;
        }
        .flatpickr-day.today { 
            border-color: #8056ff !important; 
        ...
        .flatpickr-day.inRange {
            background: rgba(128, 86, 255, 0.2) !important;
            border-color: transparent !important;
        }
        ...
        .flatpickr-time input:hover, .flatpickr-time input:focus { background: #4f4f6a !important; }
        ...
        .flatpickr-time .flatpickr-am-pm:hover { background: #8056ff !important; }
        .flatpickr-time .numInputWrapper span.arrowUp:after { border-bottom-color: white !important; }
        .flatpickr-time .numInputWrapper span.arrowDown:after { border-top-color: white !important; }
        ...
        /* Custom input styling for date field */
        ...
        .datetime-input-wrapper input.flatpickr-input {
            background: white;
        }
...
<body>
    ...
    <div id="createModal" class="modal" style="z-index: 2005;">
        <div class="modal-content" id="createModalContent" style="background:white; width: 600px; padding: 30px; border-radius: 20px; height: auto; max-height: 90vh; overflow-y: auto; overflow-x: visible; position: relative;">
            ...
            <form method="POST" enctype="multipart/form-data" id="createAssignmentForm" onsubmit="return validateAssignmentForm()">
                ...
                <div class="form-group" style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div>
                        ...
                        <div class="datetime-input-wrapper">
                            <input type="text" name="deadline" id="assignDeadline" class="form-input" required placeholder="Select date & time" readonly>
                            <i class="fa-regular fa-calendar"></i>
                        </div>
                        <div class="field-hint" id="deadlineHint">Must be a future date</div>
                    </div>
                    <div>
                ...
                <button type="submit" name="create_assignment" id="submitBtn" class="btn" style="width:100%;">
    ...
    <script>
        ...
        function initDeadlinePicker() {
            ...
            deadlinePicker = flatpickr("#assignDeadline", {
                enableTime: true,
                ...
                disableMobile: true,
                static: true,
                appendTo: document.getElementById('createModalContent'),
                position: "below",
                onChange: function(selectedDates, dateStr) {
                    formChanged = true;
                    const isValid = selectedDates.length > 0 && selectedDates[0] > new Date();
                    const input = document.getElementById('assignDeadline');
                    validateField(input, isValid);
                    document.getElementById('deadlineHint').textContent = isValid ? '✓ Deadline set' : 'Must be a future date';
                    document.getElementById('deadlineHint').className = isValid ? 'field-hint success' : 'field-hint error';
                }
            });
        }
        initDeadlinePicker();
        ...
        function validateAssignmentForm() {
            ...
            if (!deadline) errors.push('Please set a deadline.');
            else if (new Date(deadline) <= new Date()) errors.push('Deadline must be in the future.');
        ...
        function openCreateModal() { 
            ...
            // Refresh min date for Flatpickr
            if (deadlinePicker) {
                const minDate = new Date();
                minDate.setHours(minDate.getHours() + 1);
                deadlinePicker.set('minDate', minDate);
            }
            formChanged = false;
        }
        ...
        function resetCreateForm() {
            ...
            document.getElementById('deadlineHint').textContent = 'Must be a future date';
            document.getElementById('deadlineHint').className = 'field-hint';
            // Clear Flatpickr
            if (deadlinePicker) {
