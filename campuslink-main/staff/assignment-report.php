...
// 2. Find the programID from class_schedule for this course + tutGroup
$progSql = "SELECT DISTINCT programID FROM class_schedule 
            WHERE courseID = ? AND tutGroup = ? LIMIT 1";
$stmtProg = $db->prepare($progSql);
$stmtProg->bind_param("ss", $meta['courseID'], $meta['tutGroup']);
$stmtProg->execute();
$progResult = $stmtProg->get_result()->fetch_assoc();
$programID = $progResult ? $progResult['programID'] : '';

// 3. Get program name
$progNameSql = "SELECT programName FROM program WHERE programID = ?";
$stmtPN = $db->prepare($progNameSql);
$stmtPN->bind_param("s", $programID);
$stmtPN->execute();
$progNameResult = $stmtPN->get_result()->fetch_assoc();
$programName = $progNameResult ? $progNameResult['programName'] : 'Unknown Program';

// 4. Fetch All Students in this Group with LEFT JOIN to Submissions
$sql = "SELECT st.studentID, st.studentName, 
        ...
        WHERE st.programID = ? AND st.tutGroup = ?
        ORDER BY st.studentName ASC";
$stmt2 = $db->prepare($sql);
$stmt2->bind_param("iss", $assignmentID, $programID, $meta['tutGroup']);
$stmt2->execute();
