...
// Logic: Get students enrolled in this specific course (via program_course) for their current year/semester, then JOIN with attendance table to check status
$sql = "SELECT st.studentName, st.studentID, 
        COALESCE(a.status, 'Absent') as status, 
        a.scanTime
        FROM student st
        INNER JOIN program_course pc ON st.programID = pc.programID 
            AND pc.courseID = ? 
...
$stmt2->bind_param("sisssss", $meta['courseID'], $scheduleID, $date, $meta['programID'], $meta['tutGroup'], $meta['tutGroup'], $meta['tutGroup']);
$stmt2->execute();
