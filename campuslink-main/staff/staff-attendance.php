...
function syncAllAttendanceSessions($db, $currentDay) {
    // 1. Auto-CREATE sessions for ALL classes that have started today (no session exists yet)
    ...
    // 2. Auto-CLOSE ALL sessions for classes that ended 15+ minutes ago
...
syncAllAttendanceSessions($db, $currentDay);

// --- HANDLE CSV EXPORT (Must be before HTML) ---
if (isset($_POST['export_csv']) && !empty($_POST['month_filter'])) {
    $month = $_POST['month_filter'];
    ...
    fputcsv($output, ['Date', 'Course Code', 'Course Name', 'Group', 'Student Name', 'Student ID', 'Status', 'Scan Time']);
    
    // Export all students (including absent) for each conducted session
    $q = "SELECT sess.sessionDate as attendanceDate, s.courseID, c.courseName, s.tutGroup, 
                 st.studentName, st.studentID, COALESCE(a.status, 'Absent') as status, a.scanTime
          FROM attendance_sessions sess
          JOIN class_schedule s ON sess.scheduleID = s.scheduleID
          JOIN course c ON s.courseID = c.courseID
          JOIN student st ON st.programID = s.programID AND (st.tutGroup = s.tutGroup OR s.tutGroup = 'Combined')
          LEFT JOIN attendance a ON a.studentID = st.studentID AND a.scheduleID = sess.scheduleID AND a.attendanceDate = sess.sessionDate
          WHERE s.staffID = ? AND sess.sessionDate LIKE ?
          AND (sess.status = 'CLOSED' OR CONCAT(sess.sessionDate, ' ', s.endTime) < DATE_SUB(NOW(), INTERVAL 15 MINUTE))
          ORDER BY sess.sessionDate DESC, s.courseID ASC, st.studentName ASC";
          
    $param = $month . '%';
    $stmt = $db->prepare($q);
    $stmt->bind_param("ss", $staffID, $param);
    $stmt->execute();
...
$historySql = "(SELECT sess.sessionID, sess.scheduleID, sess.sessionDate as attendanceDate, 
               s.startTime, s.endTime, s.tutGroup, c.courseName, c.courseID,
                ...
                <div class="history-section">
                    <div class="history-header">
                        ...
                        <div class="filter-actions">
                            ...
                            <form method="POST">
                                <input type="hidden" name="month_filter" value="<?php echo $monthFilter; ?>">
                                <button type="submit" name="export_csv" class="btn-export" <?php if(!$monthFilter) echo 'disabled style="opacity:0.5; cursor:not-allowed;"'; ?>>
                    ...
                    <?php else: ?>
                        <table class="history-table">
                            ...
                            <tbody>
                                ...
                                ?>
                                <tr>
                                    <td><span class="date-badge"><?php echo date('M d, Y', strtotime($hist['attendanceDate'])); ?></span></td>
                                    <td>
                ...
                const row = `
                    <div class="student-row" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #f3f4f6;">
                        <div style="display:flex; align-items:center; gap:12px;">
                            <div style="width:36px; height:36px; background:#f3f4f6; border-radius:50%; display:grid; place-items:center; color:#6b7280; font-size:14px;">
                                <i class="fa-solid fa-user"></i>
                            </div>
                            <div>
