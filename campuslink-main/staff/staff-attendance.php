...
if (isset($_POST['export_csv']) && !empty($_POST['month_filter'])) {
    ...
    $q = "SELECT a.attendanceDate, s.courseID, c.courseName, s.tutGroup, st.studentName, st.studentID, a.status, a.scanTime
          FROM attendance a
          JOIN class_schedule s ON a.scheduleID = s.scheduleID
          JOIN course c ON s.courseID = c.courseID
          JOIN student st ON a.studentID = st.studentID
          WHERE s.staffID = ? AND a.attendanceDate LIKE ?
          ORDER BY a.attendanceDate DESC, s.courseID ASC, st.studentName ASC";
...
$historySql = "SELECT DISTINCT a.scheduleID, a.attendanceDate, s.startTime, s.endTime, s.tutGroup, c.courseName, c.courseID,
               (SELECT COUNT(*) FROM attendance a2 WHERE a2.scheduleID = a.scheduleID AND a2.attendanceDate = a.attendanceDate) as joinedCount,
               (SELECT COUNT(*) FROM student st WHERE st.programID = s.programID AND (st.tutGroup = s.tutGroup OR s.tutGroup = 'Combined')) as totalStudents
               FROM attendance a
               JOIN class_schedule s ON a.scheduleID = s.scheduleID
               JOIN course c ON s.courseID = c.courseID
...
if ($monthFilter) {
    $historySql .= " AND a.attendanceDate LIKE '" . $db->real_escape_string($monthFilter) . "%' ORDER BY a.attendanceDate DESC";
} else {
    $historySql .= " ORDER BY a.attendanceDate DESC LIMIT 5"; // Default: Last 5 classes
}

$stmtHist = $db->prepare($historySql);
$stmtHist->bind_param("s", $staffID);
$stmtHist->execute();
...
<head>
    ...
    <style>
        ...
        .btn-export { background: #10b981; color: white; border: none; padding: 8px 15px; border-radius: 10px; font-size: 13px; font-weight: 600; cursor: pointer; display: flex; align-items: center; gap: 6px; }
