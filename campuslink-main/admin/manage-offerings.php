...
<div class="app-bg">
    <div class="main-card">
        ...
        <aside class="sidebar">
            ...
            <nav class="sidebar-nav">
                <ul>
                    ...
                    <li>
                        <a href="manage-offerings.php" class="nav-item is-active">
        ...
        <main class="dashboard">
            ...
            <!-- Generate Offerings Section -->
            <div class="ce-admin-section">
                <div class="ce-section-title">
                    <i class="fa-solid fa-wand-magic-sparkles"></i> Generate Course Offerings
                </div>
                <p style="font-size: 13px; color: #666; margin-bottom: 15px;">
                    Auto-create offerings from programme structure for a selected term.
                </p>
                <div class="ce-gen-form">
                    <div class="ce-filter-group">
                        <label>Programme</label>
                        <select id="genProgram">
                    ...
                    <div class="ce-filter-group">
                        ...
                        <input type="number" id="genCapacity" value="40" min="1" max="500" class="ce-input" style="width: 100px;">
                    </div>
                    <button class="ce-btn ce-btn-primary" onclick="generateOfferings()">
                        <i class="fa-solid fa-plus-circle"></i> Generate Offerings
                    </button>
                </div>
                <div id="genResult" class="ce-gen-result"></div>
            </div>
...
async function loadAllDropdowns() {
    try {
        ...
        if (programs.success) {
            ...
            document.getElementById('genProgram').innerHTML += progOptions;
        ...
        if (terms.success) {
            ...
            document.getElementById('genTerm').innerHTML += termOptions;
...
async function generateOfferings() {
    const programID = document.getElementById('genProgram').value;
    const termID = document.getElementById('genTerm').value;
    const capacity = parseInt(document.getElementById('genCapacity').value) || 40;

    if (!programID || !termID) {
        showToast('Please select both programme and term', 'error');
        return;
    }
    ...
    try {
        const res = await fetch('../api/courseRegistration.php', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ action: 'createOfferings', termID, programID, capacity })
        });
        const result = await res.json();
        ...
        if (result.success) {
            ...
            resultDiv.innerHTML = `
                <div class="ce-gen-error">
                    <i class="fa-solid fa-times-circle"></i> ${result.message}
                </div>
            `;
            showToast(result.message, 'error');
        }
    } catch (err) {
        console.error('Error generating offerings:', err);
        showToast('Failed to generate offerings', 'error');
    }
}
