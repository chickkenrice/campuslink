...
$sql = "SELECT 
            ...
            COALESCE(
              NULLIF((SELECT GROUP_CONCAT(DISTINCT sc.courseID ORDER BY sc.courseID SEPARATOR ', ')
                      FROM staff_course sc WHERE sc.staffID = st.staffID), ''),
              (SELECT GROUP_CONCAT(DISTINCT cs.courseID ORDER BY cs.courseID SEPARATOR ', ')
               FROM class_schedule cs WHERE cs.staffID = st.staffID)
            ) as courseIDs
        FROM users u
