                ...
                <?php if($isSubmitted): ?>
                    <div style="border: 1px solid #e5e7eb; border-radius: 8px; padding: 12px; display: flex; align-items: center; gap: 10px; margin-bottom: 20px;">
                        ...
                        <div style="overflow: hidden;">
                            <a href="../uploads/submissions/<?php echo htmlspecialchars($data['subFile'], ENT_QUOTES, 'UTF-8'); ?>" download style="display: block; font-weight: 600; font-size: 13px; color: #1f2937; text-decoration: none; white-space: nowrap; overflow: hidden; text-overflow: ellipsis;"><?php echo htmlspecialchars($data['subFile'], ENT_QUOTES, 'UTF-8'); ?></a>
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['subFile'])) {
    ...
    $fileExt  = strtolower(pathinfo($_FILES['subFile']['name'], PATHINFO_EXTENSION));
    $fileMime = mime_content_type($_FILES['subFile']['tmp_name']);
    $fileSize = $_FILES['subFile']['size'];
    
    if (!in_array($fileExt, $allowedExts) || !in_array($fileMime, $allowedMimes)) {
        echo json_encode(['status' => 'error', 'message' => 'Invalid file type. Allowed: PDF, DOC, DOCX, PPT, PPTX, XLS, XLSX, ZIP, TXT, JPG, PNG.']);
        exit;
    }
    if ($fileSize > $maxFileSize) {
        echo json_encode(['status' => 'error', 'message' => 'File too large. Maximum allowed size is 10MB.']);
        exit;
    }
