...
/**
 * Notifications API
 * Handles fetching notifications and marking as read for students
 */
session_start();
...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($action === 'list') {
        ...
        $upcomingClasses = getUpcomingClasses($db, $studentID);
    ...
    if ($action === 'list_all') {
        ...
        $stmtTotal->bind_param("s", $studentID);
        $stmtTotal->execute();
        ...
        $stmt = $db->prepare("SELECT id, type, title, message, link, isRead, createdAt 
                              ...
                              ORDER BY isRead ASC, createdAt DESC 
                              LIMIT ? OFFSET ?");
        $stmt->bind_param("sii", $studentID, $perPage, $offset);
        $stmt->execute();
        $result = $stmt->get_result();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($action === 'markRead') {
        ...
        if ($notifId > 0) {
            ...
            $stmt->bind_param("is", $notifId, $studentID);
            $stmt->execute();
        }
