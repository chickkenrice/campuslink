...
elseif ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    switch ($action) {
        ...
        case 'create':
            ...
            // Basic validation
            if (empty($facilityID) || empty($bookingDate) || empty($startTime) || empty($endTime)) {
            ...
            // DAILY 2-HOUR CAP: Check how many minutes the user has already booked today
            $dailyCapMinutes = 120; // 2 hours per day
            $dailyStmt = $db->prepare(
                "SELECT COALESCE(SUM(TIMESTAMPDIFF(MINUTE, startTime, endTime)), 0) AS bookedMinutes
                 FROM facility_booking
                 WHERE userID = ? AND bookingDate = ? AND status = 'Active'"
            );
            $dailyStmt->bind_param("ss", $userID, $bookingDate);
            $dailyStmt->execute();
            $bookedMinutes = (int)$dailyStmt->get_result()->fetch_assoc()['bookedMinutes'];
            $remainingMinutes = $dailyCapMinutes - $bookedMinutes;

            if ($remainingMinutes <= 0) {
                echo json_encode(['success' => false, 'message' => 'Daily booking limit reached. You have already booked 2 hours today. Please try again tomorrow.']);
                exit;
            }
            if ($durationMinutes > $remainingMinutes) {
                $remHours = floor($remainingMinutes / 60);
                $remMins  = $remainingMinutes % 60;
                $remStr   = $remHours > 0 ? "{$remHours}h {$remMins}m" : "{$remMins}m";
                echo json_encode(['success' => false, 'message' => "Daily booking limit: you only have {$remStr} remaining today (2-hour daily cap)."]);
                exit;
            }
            ...
            if ($insertStmt->execute()) {
                $newBookingID = $insertStmt->insert_id;

                // Create notification for student bookings
                if ($userRole === 'student') {
                    $facNameStmt = $db->prepare("SELECT facilityName FROM facility WHERE facilityID = ?");
                    $facNameStmt->bind_param("s", $facilityID);
                    $facNameStmt->execute();
                    $facName = $facNameStmt->get_result()->fetch_assoc()['facilityName'] ?? $facilityID;

                    $notifTitle = "Booking Confirmed: " . $facName;
                    $notifMsg = date('j M Y', strtotime($bookingDate)) . ' • ' . date('g:iA', strtotime($startTime)) . '-' . date('g:iA', strtotime($endTime));
                    $notifLink = "resource-booking.php";
                    $notifStmt = $db->prepare("INSERT INTO notifications (userID, userRole, type, title, message, link) VALUES (?, 'student', 'booking_update', ?, ?, ?)");
                    $notifStmt->bind_param("ssss", $userID, $notifTitle, $notifMsg, $notifLink);
                    $notifStmt->execute();
                }

                echo json_encode(['success' => true, 'message' => 'Booking confirmed!', 'bookingID' => $newBookingID]);
            } else {
