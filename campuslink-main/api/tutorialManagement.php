 ...
 */
function getStudentProfile($db, $staffID) {
    ...
    // Verify this staff teaches a group that contains the requested student
    $authStmt = $db->prepare("SELECT 1 FROM class_schedule cs
                              JOIN student s ON cs.programID = s.programID AND cs.tutGroup = s.tutGroup
                              WHERE cs.staffID = ? AND s.studentID = ?
                              AND cs.classType IN ('Tutorial', 'Practical', 'Lab')
                              LIMIT 1");
    $authStmt->bind_param("ss", $staffID, $studentID);
    $authStmt->execute();
    $authStmt->store_result();
    if ($authStmt->num_rows === 0) {
        echo json_encode(['success' => false, 'message' => 'Unauthorized: student not in your assigned groups']);
        return;
    }
...
function removeStudent($db, $staffID, $input) {
    ...
    if ($insertStmt->execute()) {
        ...
        logActivity(
            $db,
            $_SESSION['actual_user_id'] ?? $_SESSION['user_id'],
            'STUDENT_REMOVAL',
