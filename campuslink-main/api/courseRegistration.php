 ...
 * Registration opens 4 weeks before the term start date and closes 1 day before.
 */
function autoSetRegDatesIfNeeded($db, $termID, $startDate) {
    // Check if dates are already set
    $stmt = $db->prepare("SELECT regStartDate, regEndDate FROM academic_term WHERE termID = ?");
    $stmt->bind_param("i", $termID);
    $stmt->execute();
    ...
    // Auto-compute: registration opens 4 weeks before term, closes 1 day before term
