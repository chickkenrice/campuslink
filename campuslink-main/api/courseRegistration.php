...
function getRegistrationTerm($db) {
    ...
    // Find the upcoming term with open registration window for this program
    // that matches the student's next semester
    $stmt = $db->prepare("
        ...
        WHERE t.programID = ? 
          ...
          AND t.year = ?
          AND t.semester = ?
          AND t.regStartDate IS NOT NULL 
          AND t.regEndDate IS NOT NULL
          AND CURDATE() BETWEEN t.regStartDate AND t.regEndDate
    ...
    if (!$term) {
        // Check if there's a matching term but registration not yet open
        $stmt = $db->prepare("
            SELECT t.*, p.programName 
            FROM academic_term t
            JOIN program p ON t.programID = p.programID
...
function getAvailableCourses($db) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $prereqStmt = $db->prepare("
            SELECT cp.prerequisiteID, c.courseName,
                   CASE WHEN sch.status = 'Passed' THEN 1 ELSE 0 END AS isMet
            FROM course_prerequisite cp
            JOIN course c ON cp.prerequisiteID = c.courseID
            LEFT JOIN student_course_history sch ON sch.courseID = cp.prerequisiteID 
                AND sch.studentID = ? AND sch.status = 'Passed'
            WHERE cp.courseID = ?
        ");
        $prereqStmt->bind_param("ss", $studentID, $row['courseID']);
        $prereqStmt->execute();
        ...
        $row['allPrereqsMet'] = $allPrereqsMet;
...
function registerCourse($db, $data) {
    ...
    // 5. Check prerequisites (informational — won't hard-block for now)
    $prereqWarning = checkPrerequisites($db, $studentID, $offering['courseID']);

    // 5b. Check elective limit (1 elective per year/semester group)
    ...
    if (!empty($prereqWarning)) {
        $response['prereqWarning'] = $prereqWarning;
    }
 ...
 * Returns array of unmet prerequisites (empty if all met)
 */
function checkPrerequisites($db, $studentID, $courseID) {
    $stmt = $db->prepare("
        SELECT cp.prerequisiteID, c.courseName
        FROM course_prerequisite cp
        JOIN course c ON cp.prerequisiteID = c.courseID
        LEFT JOIN student_course_history sch ON sch.courseID = cp.prerequisiteID 
            AND sch.studentID = ? AND sch.status = 'Passed'
        WHERE cp.courseID = ? AND sch.historyID IS NULL
    ");
    ...
    $unmet = [];
    while ($row = $result->fetch_assoc()) {
        $unmet[] = $row['prerequisiteID'] . ' - ' . $row['courseName'];
    }
