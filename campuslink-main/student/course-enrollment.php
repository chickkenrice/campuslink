...
// Get student info
$stmt = $db->prepare("SELECT s.studentName, s.tutGroup, s.programID, s.currentYear, s.currentSemester, p.programName 
                       FROM student s 
                ...
                <ul>
                    ...
                    <li class="nav-group is-open" data-expandable>
                        ...
                        <div class="nav-submenu" id="programme-panel">
                            ...
                            <a href="course-enrollment.php" class="nav-subitem active" style="color: var(--purple-base); font-weight: 600;">Course Enrollment</a>
        ...
        <main class="dashboard">
            ...
            <div style="margin-bottom: 20px;">
                ...
                <p style="color: var(--text-sub); font-size: 14px; margin-top: 5px;">
                    <?php echo htmlspecialchars($student['programID'] . ' - ' . $student['programName']); ?>
                    <span class="ce-badge-info" style="margin-left: 10px;">
                        Year <?php echo $student['currentYear']; ?> Semester <?php echo $student['currentSemester']; ?>
                    </span>
                </p>
            </div>
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.upcomingTerm) {
                ...
                let msg = `<strong>Registration Closed</strong> — No active registration window.`;
                if (ut.regStartDate) {
                    msg += `<br><small>Next registration for Year ${ut.year} Semester ${ut.semester}: ${formatDate(ut.regStartDate)} — ${formatDate(ut.regEndDate)}</small>`;
                } else {
                    msg += `<br><small>Next term (Year ${ut.year} Semester ${ut.semester}) has no registration dates set yet.</small>`;
                }
                banner.innerHTML = `<i class="fa-solid fa-circle-xmark"></i><div>${msg}</div>`;
            } else {
                banner.innerHTML = `<i class="fa-solid fa-circle-info"></i><div><strong>No Registration Period</strong> — ${result.message}</div>`;
            }
...
function renderCourseCard(course, isElective = false) {
    ...
    if (course.prerequisites && course.prerequisites.length > 0) {
        const prereqItems = course.prerequisites.map(p => {
            const icon = p.isMet == 1 
                ? '<i class="fa-solid fa-check" style="color: #059669;"></i>' 
                : '<i class="fa-solid fa-xmark" style="color: #dc2626;"></i>';
            return `<span class="ce-prereq-item">${icon} ${p.prerequisiteID}</span>`;
        }).join('');
        prereqHTML = `<div class="ce-prereq-list"><span class="ce-prereq-label">Prerequisites:</span> ${prereqItems}</div>`;
    }
    ...
    if (isRegistered) {
        ...
        statusText = 'Full';
    } else if (!course.allPrereqsMet && course.prerequisites.length > 0) {
        statusClass = 'ce-status-prereq';
...
function updateSummaryCards(courses) {
    ...
    const totalCr = courses.reduce((sum, c) => sum + parseInt(c.creditHours), 0);
...
async function registerCourse(offeringID, courseID) {
    ...
    try {
        ...
        if (result.success) {
            ...
            if (result.prereqWarning && result.prereqWarning.length > 0) {
                showToast('Warning: Unmet prerequisites — ' + result.prereqWarning.join(', '), 'warning');
            }
            loadAvailableCourses();
            loadRegisteredCourses();
        } else {
