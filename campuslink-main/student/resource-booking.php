        ...
        <main class="dashboard">
            ...
            <section class="welcome-card">
                <div class="welcome-details">
                    ...
                    <p class="welcome-text">Book facilities for study, sports, and more</p>
                </div>
            </section>
            ...
            <section class="form-card" style="margin-top: 20px;">
                ...
                <div class="rb-steps">
                    ...
                    <!-- Step 2: Date & Time Selection (hidden until facility selected) -->
                    <div id="booking-panel" style="display:none;">
                        ...
                        <div class="rb-booking-form">
                            ...
                            <div id="purpose-section" style="display:none;">
                                <div class="rb-form-group" style="margin-top: 15px;">
                                    <label for="rb-purpose">4. Purpose</label>
                                    <input type="text" id="rb-purpose" placeholder="e.g. Group study session, Badminton practice..." maxlength="255">
                                </div>
                                <button id="btn-book" class="rb-btn-submit" onclick="submitBooking()">
            ...
            <section class="form-card" style="margin-top: 20px;">
                ...
                <div id="my-bookings-container" style="display:none; margin-top:15px;">
                    ...
                    <div class="table-container">
                        <table class="student-table" id="my-bookings-table">
                            <thead>
                                <tr>
                                    <th>Facility</th>
                                    <th>Date</th>
...
<script>
    ...
    let slotData = [];
    let maxDurationSlots = 4; // default 2 hours = 4 x 30min
    ...
    function toggleSlot(idx) {
        if (selectedSlots.start === null) {
            ...
            if (idx <= selectedSlots.start) {
                ...
                // Second click = end boundary (user clicks the end time they want)
                // The clicked slot's START time is the end boundary
                // Included slots: start to idx-1
                const lastSlot = idx - 1;
                const slotCount = lastSlot - selectedSlots.start + 1;
                
                // Minimum 1 hour (2 slots), Maximum 2 hours (4 slots)
                if (slotCount < 2) {
                    showBookingMsg('Minimum booking duration is 1 hour. Please select at least 1 hour.', 'error');
                    return;
                }
                if (slotCount > 4) {
                    showBookingMsg('Maximum booking duration is 2 hours. Please select fewer slots.', 'error');
                    return;
                }
                ...
                selectedSlots.endClicked = idx; // Store clicked index for end time display
            }
            highlightSlots();
            updateTimeInfo();
        }
    ...
    function highlightSlots() {
        document.querySelectorAll('.rb-slot').forEach(btn => {
        ...
        if (selectedSlots.start !== null) {
            for (let i = selectedSlots.start; i <= selectedSlots.end; i++) {
                ...
                if (btn) {
                    btn.classList.add(i === selectedSlots.start || i === selectedSlots.end ? 'selected' : 'in-range');
                }
            }
        }
    }
    ...
    function updateTimeInfo() {
        ...
        // Use the clicked slot's start time as the end boundary
        // Fallback to selectedSlots.end + 1 if endClicked is not set (shouldn't happen)
        const endIndex = selectedSlots.endClicked !== null && selectedSlots.endClicked !== undefined 
            ? selectedSlots.endClicked 
            : selectedSlots.end + 1;
        const endTime = slotData[endIndex].start;
    ...
    async function submitBooking() {
        ...
        if (!selectedFacility || selectedSlots.start === null || !date) {
            showBookingMsg('Please complete all steps.', 'error');
            return;
        }
        if (!purpose) {
            showBookingMsg('Please enter a purpose for your booking.', 'error');
            return;
        }
        ...
        try {
            ...
            if (result.success) {
                ...
                // Refresh slots and bookings after a short delay to ensure DB is updated
                setTimeout(() => {
                ...
                document.getElementById('rb-purpose').value = '';
                selectedSlots = { start: null, end: null, endClicked: null };
            } else {
    ...
    function renderMyBookings(bookings) {
        ...
        if (bookings.length === 0) {
            tbody.innerHTML = '<tr><td colspan="6" style="text-align:center; padding:30px; color:var(--text-sub);">No bookings found</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = bookings.map(b => {
            ...
            return `<tr>
                <td><strong>${b.facilityName}</strong><br><small style="color:var(--text-sub);">${b.type} &bull; ${b.location}</small></td>
                <td>${b.bookingDate}</td>
                <td>${b.startTime.substring(0,5)} - ${b.endTime.substring(0,5)}</td>
                <td>${b.purpose}</td>
                <td><span class="rb-status ${statusClass}">${b.status}</span></td>
                <td>${canCancel ? `<button class="rb-btn-cancel" onclick="cancelBooking(${b.bookingID})"><i class="fa-solid fa-xmark"></i> Cancel</button>` : '-'}</td>
            </tr>`;
        }).join('');
    }
    ...
    async function cancelBooking(bookingID) {
        ...
        try {
            const res = await fetch('../api/resource_booking.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ action: 'cancel', bookingID: bookingID })
            });
            const result = await res.json();

            if (result.success) {
                loadMyBookings();
                // Refresh slots if viewing same date
                const dateInput = document.getElementById('rb-date');
                if (dateInput.value) {
                    dateInput.dispatchEvent(new Event('change'));
                }
    ...
    async function loadModalScheduleTypes() {
        try {
            ...
            if (result.success) {
                ...
                // Reset options keeping the first one
