...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['scheduleID'])) {
    ...
    if (!isset($_POST['attend_csrf']) || $_POST['attend_csrf'] !== $_SESSION['attend_csrf']) {
        ...
        $msgType = "error";
    // WiFi/Network check — students must be on campus network
    } elseif (isset($_POST['digit'])) {
        $studentIP = $_SERVER['REMOTE_ADDR'];
        // Also check forwarded IP for proxy setups
        if (!empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
            $studentIP = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR'])[0];
        }
        if ($studentIP !== ALLOWED_WIFI_IP && $studentIP !== '::1' && $studentIP !== '127.0.0.1') {
            $msg = "You must be connected to the campus WiFi to take attendance. Please connect and try again.";
            $msgType = "wifi";
        } else {
...
<head>
    ...
    <style>
        ...
        .toast.wifi { background: linear-gradient(135deg, #fffbeb 0%, #fef3c7 100%); color: #92400e; border: 1px solid #fde68a; }
        .toast.wifi .toast-icon { background: #fffbeb; color: #f59e0b; }
    ...
    <div class="toast-container">
        <div class="toast <?php echo $msgType; ?>">
            ...
            <div class="toast-content">
                <div class="toast-title"><?php echo $msgType === 'wifi' ? 'Not on Campus Network' : ($msgType === 'error' ? 'Attendance Failed' : 'Attendance Recorded'); ?></div>
