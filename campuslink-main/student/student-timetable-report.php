<?php
// File: student/student-timetable-report.php
// Official Class Timetable Report for Students
...
// Get student info
$stmt = $db->prepare("SELECT s.studentID, s.studentName, s.tutGroup, s.programID, s.currentYear, s.currentSemester, 
                              p.programName, p.faculty
                       FROM student s
                       LEFT JOIN program p ON s.programID = p.programID
                       WHERE s.studentID = ? OR s.userID = ?");
...
$tutGroup    = $student['tutGroup'];
$programID   = $student['programID'];
$programName = $student['programName'] ?? $programID;
$faculty     = $student['faculty'] ?? '';
$currentYear = $student['currentYear'] ?? 1;
$currentSemester = $student['currentSemester'] ?? 1;

// Get active academic term
$termStmt = $db->prepare("SELECT * FROM academic_term 
                          WHERE programID = ? AND year = ? AND semester = ? AND status = 'Active' 
                          LIMIT 1");
$termStmt->bind_param("sii", $programID, $currentYear, $currentSemester);
$termStmt->execute();
$term = $termStmt->get_result()->fetch_assoc();

$academicYear = $term['academicYear'] ?? 'N/A';
$semester     = $term['semester'] ?? $currentSemester;
...
$sql = "SELECT cs.courseID, cs.day, cs.startTime, cs.endTime, cs.classType, cs.facilityID,
               c.courseName, c.creditHours, f.facilityName, st.staffName
        FROM class_schedule cs
        JOIN course c ON cs.courseID = c.courseID
        JOIN facility f ON cs.facilityID = f.facilityID
        JOIN staff st ON cs.staffID = st.staffID
        JOIN program_course pc ON cs.courseID = pc.courseID AND cs.programID = pc.programID
        WHERE cs.tutGroup = ? 
        AND cs.programID = ?
        AND pc.year = ?
        AND pc.semester = ?
...
$stmt = $db->prepare($sql);
$stmt->bind_param("ssii", $tutGroup, $programID, $currentYear, $currentSemester);
$stmt->execute();
$result = $stmt->get_result();

$scheduleData = [];
$courseSummary = []; // For course summary section

while ($row = $result->fetch_assoc()) {
    $scheduleData[] = $row;
    // Build unique course list for summary
    if (!isset($courseSummary[$row['courseID']])) {
        $courseSummary[$row['courseID']] = [
    ...
    if (!in_array($row['classType'], $courseSummary[$row['courseID']]['classTypes'])) {
        $courseSummary[$row['courseID']]['classTypes'][] = $row['classType'];
    }
}
...
// Intake (derived from studentID prefix or academic year)
$intake = $academicYear;
?>
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        .class-block.is-Lecture { background: #dbeafe; border-left: 3px solid #2563eb; }
        .class-block.is-Practical { background: #dcfce7; border-left: 3px solid #16a34a; }
        .class-block.is-Tutorial { background: #fef3c7; border-left: 3px solid #d97706; }
        ...
        .class-block .cb-staff { color: #666; display: block; font-style: italic; margin-top: 1px; }
...
<body class="timetable-report-page">
    ...
    <div class="page" style="width: 297mm; min-height: 210mm;">
        ...
        <div class="header">
            ...
            <div class="report-title">Official Class Timetable</div>
        </div>
        ...
        <div class="info-grid">
            ...
            <div class="info-cell">
                ...
                <span class="value"><?php echo htmlspecialchars($studentID); ?></span>
            </div>
            <div class="info-cell">
                <span class="label">Programme</span>
                <span class="value"><?php echo htmlspecialchars($programID); ?></span>
            </div>
            <div class="info-cell">
                <span class="label">Tutorial Group</span>
                <span class="value"><?php echo htmlspecialchars($tutGroup); ?></span>
            </div>
            <div class="info-cell">
            ...
            <thead>
                <tr>
                    ...
                    <?php for ($h = $startHour; $h < $endHour; $h++): ?>
                        <th colspan="2"><?php echo sprintf("%02d:00", $h); ?></th>
                    <?php endfor; ?>
                </tr>
            </thead>
            <tbody>
                    ...
                    <?php for ($i = 0; $i < $totalSlots; $i++): ?>
                        <?php if (isset($scheduleMatrix[$day][$i])): ?>
                            ...
                            <?php 
                                ...
                                $suffix = ($info['classType'] === 'Lecture') ? '(L)' : ($info['classType'] === 'Practical' ? '(P)' : '(T)');
                            ?>
                            <td colspan="<?php echo $slot['colspan']; ?>">
                                <div class="class-block <?php echo $typeClass; ?>">
                                    ...
                                    <span class="cb-staff"><?php echo htmlspecialchars($info['staffName']); ?></span>
                                </div>
                            </td>
                        <?php else: ?>
            ...
            <table class="summary-table">
                <thead>
                    <tr>
                        ...
                        <th style="width: 42%;">Course Name</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td><?php echo htmlspecialchars($course['staffName']); ?></td>
                    </tr>
                    <?php endforeach; ?>
                    <tr class="total-row">
                        <td colspan="3" style="text-align: right; padding-right: 15px;">Total Credit Hours</td>
                        <td style="text-align: center;"><?php echo $totalCredits; ?></td>
                        <td></td>
                    </tr>
                </tbody>
            </table>
        ...
        <div class="report-note">
            <strong>Note:</strong> This timetable is subject to change. Please check the system regularly for any updates or replacement classes.
        </div>
        ...
        <div class="footer">
            ...
            <div class="signature">
                <div class="sig-line">
                    <?php echo htmlspecialchars($studentName); ?><br>
                    <span style="font-weight: 400; font-size: 11px; color: #555;">Student</span>
                </div>
        ...
        <div class="page-footer">
            ...
            <span>CampusLink University - Official Class Timetable</span>
        </div>
    </div>
