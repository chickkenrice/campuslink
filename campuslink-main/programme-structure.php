...
// 1. Get Student's Programme ID from their profile
$stmt = $db->prepare("SELECT programme FROM student WHERE studentID = ?");
...
// Extract just the ID "RSD"
$progString = $student['programme']; 
$progID = explode(':', $progString)[0]; 

// 2. Fetch Program Details (Name & Faculty) from the new PROGRAM table
$pStmt = $db->prepare("SELECT programName, faculty FROM program WHERE programID = ?");
$pStmt->bind_param("s", $progID);
$pStmt->execute();
$progDetails = $pStmt->get_result()->fetch_assoc();

// Fallback if program details missing
$progName = $progDetails['programName'] ?? $progString;
$faculty = $progDetails['faculty'] ?? 'FOCS';
