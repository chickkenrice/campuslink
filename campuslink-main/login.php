...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // --- 1. STUDENT LOGIN ---
    if ($role === 'student') {
        $stmt = $db->prepare("SELECT s.studentID, s.studentName, s.userID FROM student s WHERE s.studentID = ?");
    ...
    elseif ($role === 'staff') {
        $stmt = $db->prepare("SELECT st.staffID, st.staffName, st.userID FROM staff st WHERE st.staffID = ?");
        $stmt->bind_param("s", $userID);
        $stmt->execute();
        $user = $stmt->get_result()->fetch_assoc();
    ...
    // --- 3. ADMIN LOGIN ---
    elseif ($role === 'admin') {
        $stmt = $db->prepare("SELECT a.adminID, a.adminName, a.userID FROM admin a WHERE a.adminID = ?");
        $stmt->bind_param("s", $userID);
        $stmt->execute();
        $user = $stmt->get_result()->fetch_assoc();
    ...
    <div class="login-card">
        ...
        <form method="POST" id="loginForm" novalidate>
            ...
            <div class="form-group">
                ...
                <input type="text" id="studentID" name="studentID" placeholder="ID (e.g. 23WP..., STAFF01, ADMIN01)" autofocus>
