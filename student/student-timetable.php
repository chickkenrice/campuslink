...
<script>
    ...
    function displayTermInfo() {
        ...
        const _fmtTermDate = (ds) => {
            const [ty,tm,td] = ds.split('-');
            const wd = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][new Date(ds+'T00:00:00').getDay()];
            return `${wd}, ${td}-${tm}-${ty}`;
        };
        const startDate = _fmtTermDate(actualStartDate);
        const endDate = _fmtTermDate(actualEndDate);
