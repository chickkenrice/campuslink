...
if ($viewingId) {
    $stmt = $db->prepare("SELECT id, title, body, attachment, announcement_type, is_pinned, created_at FROM announcements WHERE id = ? AND target_audience IN ('all', 'staff')");
    ...
    <div class="main-card">
        ...
        <main class="dashboard">
            ...
            ?>
                ...
                <a href="staff-dashboard.php" class="back-btn">
                ...
                <div class="ann-detail-card">
                    ...
                    <div class="ann-detail-body">
                        ...
                        <?php
                            $annAttachments = [];
                            if (!empty($singleAnnouncement['attachment'])) {
                                $decoded = json_decode($singleAnnouncement['attachment'], true);
                                $annAttachments = is_array($decoded) ? $decoded : [$singleAnnouncement['attachment']];
                            }
                        ?>
                        <?php if (!empty($annAttachments)): ?>
                        <div style="margin-top: 20px; padding-top: 16px; border-top: 1px dashed #e2e8f0; display:flex; flex-wrap:wrap; gap:8px;">
                            <?php foreach ($annAttachments as $attFile): ?>
                            <a href="../uploads/announcements/<?php echo htmlspecialchars(rawurlencode($attFile)); ?>" target="_blank" style="display: inline-flex; align-items: center; gap: 8px; padding: 10px 18px; background: #ede9fe; color: #7c3aed; border-radius: 10px; font-size: 14px; font-weight: 600; text-decoration: none;">
                                <i class="fa-solid fa-paperclip"></i> <?php echo htmlspecialchars(preg_replace('/^\d+_\d+_/', '', preg_replace('/^\d+_/', '', $attFile))); ?>
                            </a>
                            <?php endforeach; ?>
                        </div>
                        <?php endif; ?>
