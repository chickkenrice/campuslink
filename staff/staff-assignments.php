...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['create_assignment'])) {
    ...
    if (empty($errors)) {
        ...
        if ($stmt->execute()) {
            $newAssignID = $db->insert_id;
            
            // Create notifications for enrolled students
            $studentsSql = "SELECT DISTINCT st.studentID 
                           FROM student st
            ...
            $stmtStudents = $db->prepare($studentsSql);
            $stmtStudents->bind_param("ssss", $courseID, $targetGroup, $targetGroup, $targetGroup);
            $stmtStudents->execute();
            $studentsResult = $stmtStudents->get_result();
            
            $notifStmt = $db->prepare("INSERT INTO notifications (userID, userRole, type, title, message, link) VALUES (?, 'student', 'new_assignment', ?, ?, ?)");
            $notifTitle = "New Assignment: " . $title;
            $notifMsg = "Due " . date('j M Y', strtotime($deadline));
            $notifLink = "student-assignments.php";
            
            while ($stRow = $studentsResult->fetch_assoc()) {
                $notifStmt->bind_param("ssss", $stRow['studentID'], $notifTitle, $notifMsg, $notifLink);
                $notifStmt->execute();
            }
...
// 3b. Handle Delete Assignment (only if no submissions)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['delete_assignment'])) {
    $assignID = intval($_POST['assignmentID']);
    
    // Verify ownership and check submission count
    $checkStmt = $db->prepare("SELECT a.assignmentID, a.attachmentPath, 
        (SELECT COUNT(*) FROM submissions s WHERE s.assignmentID = a.assignmentID) as submitCount 
        FROM assignments a WHERE a.assignmentID = ? AND a.staffID = ?");
    $checkStmt->bind_param("is", $assignID, $staffID);
    $checkStmt->execute();
    $assignData = $checkStmt->get_result()->fetch_assoc();
    
    if ($assignData && $assignData['submitCount'] == 0) {
        // Delete attachment file if exists
        if ($assignData['attachmentPath']) {
            $filePath = '../uploads/assignments/' . $assignData['attachmentPath'];
            if (file_exists($filePath)) unlink($filePath);
        }
        // Delete from database
        $delStmt = $db->prepare("DELETE FROM assignments WHERE assignmentID = ?");
        $delStmt->bind_param("i", $assignID);
        if ($delStmt->execute()) {
            $_SESSION['assign_msg'] = "Assignment deleted successfully.";
            $_SESSION['assign_msg_type'] = "success";
        } else {
            $_SESSION['assign_msg'] = "Failed to delete assignment.";
            $_SESSION['assign_msg_type'] = "error";
        }
    } else {
        $_SESSION['assign_msg'] = "Cannot delete: Assignment has submissions or doesn't exist.";
        $_SESSION['assign_msg_type'] = "error";
    }
    header("Location: staff-assignments.php");
    exit;
...
// 3c. Handle Edit Assignment
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['edit_assignment'])) {
    $assignID = intval($_POST['assignmentID']);
    $errors = [];
    
    // Verify ownership
    $checkStmt = $db->prepare("SELECT a.*, (SELECT COUNT(*) FROM submissions s WHERE s.assignmentID = a.assignmentID) as submitCount 
        FROM assignments a WHERE a.assignmentID = ? AND a.staffID = ?");
    $checkStmt->bind_param("is", $assignID, $staffID);
    $checkStmt->execute();
    $existingAssign = $checkStmt->get_result()->fetch_assoc();
    ...
    if (!$existingAssign) {
        ...
        elseif (strlen($title) > 100) $errors[] = "Title cannot exceed 100 characters.";
        
        // Validate description
        $desc = trim($_POST['description'] ?? '');
        if (strlen($desc) < 10) $errors[] = "Description must be at least 10 characters.";
        
        // Validate deadline (can only extend if submissions exist)
        $deadlineDate = $_POST['deadline_date'] ?? '';
        $deadlineTime = $_POST['deadline_time'] ?? '23:59';
        $newDeadline = $deadlineDate . ' ' . $deadlineTime;
        
        if (empty($deadlineDate)) {
            $errors[] = "Please set a deadline date.";
        } else {
            $newDeadlineTs = strtotime($newDeadline);
            $oldDeadlineTs = strtotime($existingAssign['deadline']);
            
            // If submissions exist, can only extend deadline (not shorten)
            if ($existingAssign['submitCount'] > 0 && $newDeadlineTs < $oldDeadlineTs) {
                $errors[] = "Cannot shorten deadline when submissions exist. You can only extend it.";
            }
        }
        
        // Handle new attachment (optional)
        $newFilePath = $existingAssign['attachmentPath']; // Keep existing by default
        if (isset($_FILES['attachment']) && $_FILES['attachment']['error'] == 0) {
            $allowedTypes = ['pdf', 'doc', 'docx', 'txt', 'zip', 'rar', 'png', 'jpg', 'jpeg'];
            ...
            if (!in_array($fileExt, $allowedTypes)) {
                ...
                if (move_uploaded_file($_FILES['attachment']['tmp_name'], $uploadDir . $fileName)) {
                    // Delete old file
                    if ($existingAssign['attachmentPath']) {
                        $oldFile = $uploadDir . $existingAssign['attachmentPath'];
        ...
        // Update if no errors
        if (empty($errors)) {
            $updateStmt = $db->prepare("UPDATE assignments SET title = ?, description = ?, deadline = ?, attachmentPath = ? WHERE assignmentID = ?");
            $updateStmt->bind_param("ssssi", $title, $desc, $newDeadline, $newFilePath, $assignID);
            if ($updateStmt->execute()) {
                $_SESSION['assign_msg'] = "Assignment updated successfully!";
                $_SESSION['assign_msg_type'] = "success";
                header("Location: staff-assignments.php");
                exit;
            } else {
...
$courseFilterSql = "SELECT DISTINCT a.courseID, c.courseName 
                    FROM assignments a 
                    JOIN course c ON a.courseID = c.courseID 
                    WHERE a.staffID = ? 
                    ORDER BY c.courseName";
$stmtCourses = $db->prepare($courseFilterSql);
$stmtCourses->bind_param("s", $staffID);
$stmtCourses->execute();
$teachingCourses = $stmtCourses->get_result()->fetch_all(MYSQLI_ASSOC);
...
// 5. Fetch Assignments with archive/course filter
$courseFilter = ($selectedCourse !== 'all') ? " AND a.courseID = '" . $db->real_escape_string($selectedCourse) . "'" : "";
$archiveFilter = $showArchived ? "" : " AND a.isArchived = 0";

$assignQuery = "SELECT a.*, c.courseName, 
    (SELECT COUNT(*) FROM submissions s WHERE s.assignmentID = a.assignmentID) as submitCount 
    FROM assignments a 
    JOIN course c ON a.courseID = c.courseID 
    WHERE a.staffID = '$staffID' $courseFilter $archiveFilter
    ORDER BY a.createdAt DESC";
$assignments = $db->query($assignQuery);

// Fetch archived count for badge
$archivedCountQuery = "SELECT COUNT(*) as cnt FROM assignments WHERE staffID = '$staffID' AND isArchived = 1";
$archivedCount = $db->query($archivedCountQuery)->fetch_assoc()['cnt'];

// Store assignments in array for limit logic
$allAssignments = [];
...
<head>
    ...
    <style>
        ...
        .btn-icon-sm { width: 30px; height: 30px; border: none; background: #f3f4f6; color: #6b7280; border-radius: 8px; cursor: pointer; transition: 0.2s; display: grid; place-items: center; font-size: 12px; }
        .btn-icon-sm:hover { background: #e5e7eb; color: #1f2937; }
        ...
        .filter-select { padding: 10px 14px; border: 1px solid #e5e7eb; border-radius: 10px; font-size: 13px; font-weight: 600; color: #4b5563; background: white; cursor: pointer; outline: none; min-width: 180px; }
        ...
        .load-more-btn { background: white; border: 1px solid #e5e7eb; padding: 12px 24px; border-radius: 12px; font-size: 13px; font-weight: 600; color: #6b7280; cursor: pointer; transition: all 0.2s; display: inline-flex; align-items: center; gap: 8px; }
        .load-more-btn:hover { background: #f3efff; border-color: #8056ff; color: #8056ff; }
        .btn-archive { padding: 4px 8px; border: none; background: #f3f4f6; color: #6b7280; border-radius: 6px; cursor: pointer; font-size: 10px; font-weight: 600; transition: 0.2s; }
        .btn-archive:hover { background: #fef3c7; color: #d97706; }
        .assign-card.archived { opacity: 0.7; }
        .assign-card.archived::before { background: #9ca3af; }

        /* Toast notifications */
        .toast-container { position: fixed; top: 20px; right: 20px; z-index: 9999; display: flex; flex-direction: column; gap: 8px; }
        .toast { padding: 13px 18px; border-radius: 12px; font-size: 13px; font-family: 'Inter', sans-serif; font-weight: 600; display: flex; align-items: center; gap: 10px; box-shadow: 0 8px 24px rgba(0,0,0,0.12); animation: toastSlideIn 0.3s cubic-bezier(0.34,1.56,0.64,1); min-width: 260px; max-width: 360px; }
        ...
        @keyframes toastFadeOut { from { opacity: 1; transform: translateX(0); } to { opacity: 0; transform: translateX(110%); } }
    </style>
</head>
<body>
    <div class="app-bg">
        <div class="main-card">
            <?php $currentPage = 'assignments'; include __DIR__ . '/../includes/staffMaster.php'; ?>
            ...
            <main class="dashboard">
                ...
                <div class="action-bar">
                    <h2 class="section-title"><?php echo $showArchived ? 'All Assignments' : 'Active Tasks'; ?> <span style="font-size: 13px; font-weight: 500; color: #9ca3af;">(<?php echo $totalAssignments; ?>)</span></h2>
                ...
                <div class="filter-bar">
                    ...
                    <div class="archive-toggle">
                        ...
                        <span class="archive-count"><?php echo $archivedCount; ?></span>
                    </div>
                    <?php else: ?>
                    <div class="archive-toggle">
                        <label class="toggle-switch">
                ...
                <div class="assign-grid" id="assignGrid">
                    <?php if($totalAssignments == 0): ?>
                        <div style="grid-column:1/-1; text-align:center; padding:50px; color:#9ca3af;"><i class="fa-solid fa-folder-open" style="font-size:40px; margin-bottom:10px;"></i><p>No assignments found.</p></div>
                    <?php else: 
                        $cardIndex = 0;
                        foreach($allAssignments as $row): 
                            $cardIndex++;
                            $isHidden = $cardIndex > $defaultLimit;
                            $date = date('d M Y', strtotime($row['deadline']));
                            $isExpired = strtotime($row['deadline']) < time();
                            $isArchivedItem = $row['isArchived'] == 1;
                        ?>
                        <article class="assign-card <?php echo $isArchivedItem ? 'archived' : ''; ?>" <?php echo $isHidden ? 'style="display:none;"' : ''; ?>>
                            <div class="ac-header">
                                <div><span class="ac-course"><?php echo $row['courseID']; ?></span><span class="ac-group-badge"><?php echo $row['tutGroup']; ?></span></div>
                                <div style="display:flex; align-items:center; gap:8px;">
                                    <?php if($isArchivedItem): ?>
                                        <span style="font-size:11px; font-weight:700; color:#9ca3af; background:#f3f4f6; padding:4px 8px; border-radius:6px;">ARCHIVED</span>
                                    ...
                                    <?php endif; ?>
                                    <button onclick="toggleArchiveItem(<?php echo $row['assignmentID']; ?>, <?php echo $isArchivedItem ? '0' : '1'; ?>)" class="btn-archive" title="<?php echo $isArchivedItem ? 'Unarchive' : 'Archive'; ?>">
                                        <i class="fa-solid <?php echo $isArchivedItem ? 'fa-box-open' : 'fa-box-archive'; ?>"></i>
                                    </button>
                                    <button onclick="openEditModal(<?php echo htmlspecialchars(json_encode($row), ENT_QUOTES, 'UTF-8'); ?>)" class="btn-icon-sm" title="Edit Assignment"><i class="fa-solid fa-pen"></i></button>
                                    <?php if($row['submitCount'] == 0): ?>
                                    <button onclick="confirmDelete(<?php echo $row['assignmentID']; ?>, '<?php echo addslashes($row['title']); ?>')" class="btn-icon-sm btn-icon-danger" title="Delete Assignment"><i class="fa-solid fa-trash"></i></button>
                                    <?php else: ?>
                                    <button class="btn-icon-sm btn-icon-disabled" title="Cannot delete: Has submissions" disabled><i class="fa-solid fa-trash"></i></button>
                                    <?php endif; ?>
                                </div>
                            </div>
                        ...
                        if ($totalAssignments > $defaultLimit): ?>
                            <div style="grid-column: 1/-1; text-align: center; padding: 20px;" id="loadMoreContainer">
                                <button class="load-more-btn" onclick="loadMore()">
                                    <i class="fa-solid fa-chevron-down"></i> 
                                    Show All (<?php echo $totalAssignments - $defaultLimit; ?> more)
                                </button>
                            </div>
                        <?php endif;
                    endif; ?>
    ...
    <div id="editModal" class="modal" style="z-index: 2005;">
        <div class="modal-content" style="background:white; width: 600px; padding: 30px; border-radius: 20px; height: auto; max-height: 90vh; overflow-y: auto;">
            <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:20px;">
                <h2 style="margin:0;">Edit Assignment</h2>
                <button type="button" class="close-btn" onclick="closeEditModal()"><i class="fa-solid fa-xmark"></i></button>
            </div>
            <form method="POST" enctype="multipart/form-data" id="editAssignmentForm">
                ...
                <div class="form-group">
                    ...
                    <input type="text" id="editCourseGroup" class="form-input" disabled style="background:#f9fafb;">
                </div>
                
                <div class="form-group">
                    <label class="form-label">Assignment Title <span style="color:#ef4444;">*</span></label>
                    <input type="text" name="title" id="editTitle" class="form-input" required minlength="3" maxlength="100">
                    <div class="field-hint"><span id="editTitleCounter">0</span>/100 characters</div>
                </div>
                
                <div class="form-group">
                    <label class="form-label">Instructions / Description <span style="color:#ef4444;">*</span></label>
                    <textarea name="description" id="editDesc" class="form-input" rows="4" required minlength="10"></textarea>
                    <div class="field-hint"><span id="editDescCounter">0</span> characters</div>
                </div>
                
                <div class="form-group" style="display:grid; grid-template-columns: 1fr 1fr; gap:15px;">
                    <div>
                        <label class="form-label">Deadline Date <span style="color:#ef4444;">*</span></label>
                        <input type="date" name="deadline_date" id="editDeadlineDate" class="form-input" required>
                        <div class="field-hint" id="editDeadlineHint"></div>
                    </div>
                    <div>
                        <label class="form-label">Deadline Time <span style="color:#ef4444;">*</span></label>
                        <input type="time" name="deadline_time" id="editDeadlineTime" class="form-input" required>
                    </div>
                </div>
                
                <div class="form-group">
                    <label class="form-label">Replace Attachment (Optional)</label>
                    <div id="editCurrentFile" style="font-size:12px; color:#6b7280; margin-bottom:8px;"></div>
                    <input type="file" name="attachment" id="editFile" class="form-input" accept=".pdf,.doc,.docx,.txt,.zip,.rar,.png,.jpg,.jpeg">
                    <div class="field-hint">Leave empty to keep current file</div>
                </div>
                
                <div id="editFormError" class="form-error" style="display:none;"></div>
                <div id="editFormWarning" style="background:#fffbeb; border:1px solid #fde68a; color:#92400e; padding:12px; border-radius:10px; margin-bottom:15px; font-size:13px; display:none;"></div>
    ...
    <div id="deleteModal" class="modal" style="z-index: 2010;">
        <div class="modal-content" style="background:white; width: 400px; padding: 30px; border-radius: 20px; text-align:center;">
            <div style="width:60px; height:60px; background:#fee2e2; border-radius:50%; display:grid; place-items:center; margin:0 auto 20px;">
                <i class="fa-solid fa-trash" style="font-size:24px; color:#dc2626;"></i>
            </div>
            <h3 style="margin:0 0 10px;">Delete Assignment?</h3>
            <p style="color:#6b7280; font-size:14px; margin-bottom:25px;">This will permanently delete "<span id="deleteAssignTitle"></span>". This action cannot be undone.</p>
            <form method="POST" id="deleteForm">
                <input type="hidden" name="delete_assignment" value="1">
                <input type="hidden" name="assignmentID" id="deleteAssignmentID">
                <div style="display:flex; gap:10px;">
                    <button type="button" onclick="closeDeleteModal()" class="btn" style="flex:1; background:#f3f4f6; color:#374151;">Cancel</button>
                    <button type="submit" class="btn" style="flex:1; background:#dc2626;">Delete</button>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        // Toggle archive status for a single assignment
        function toggleArchiveItem(assignmentID, archiveStatus) {
            fetch('../api/staff-assignments-api.php', {
                method: 'POST',
                headers: { 'Content-Type': 'application/x-www-form-urlencoded' },
                body: `action=toggleArchive&assignmentID=${assignmentID}&isArchived=${archiveStatus}`
            })
            .then(r => r.json())
            .then(data => {
                if (data.status === 'success') {
                    ...
                    showToast('Error: ' + (data.message || 'Could not update archive status'), 'error');
                }
            })
            .catch(() => showToast('Error updating archive status', 'error'));
        }
        
        // Load more - show all cards
        function loadMore() {
            document.querySelectorAll('#assignGrid .assign-card').forEach(card => {
                card.style.display = '';
            });
        ...
        document.addEventListener('submit', function(e) {
            if (e.target && e.target.classList.contains('gc-grading-form')) {
                ...
                .then(data => {
                     ...
                     } else { showToast('Error saving grade.', 'error'); }
                });
            }
        });
        ...
        window.onclick = function(e) { 
            if(e.target.classList.contains('modal')) {
                ...
                } else if (e.target.id === 'editModal' && editFormChanged) {
                    if (confirm('You have unsaved changes. Are you sure you want to close?')) {
                        e.target.style.display = 'none';
                    }
        ...
        function openEditModal(data) {
            ...
            document.getElementById('editCourseGroup').value = data.courseID + ' - ' + data.courseName + ' (' + data.tutGroup + ')';
            document.getElementById('editTitle').value = data.title;
            document.getElementById('editDesc').value = data.description;
            document.getElementById('editTitleCounter').textContent = data.title.length;
            document.getElementById('editDescCounter').textContent = data.description.length;
            
            // Parse deadline
            const deadline = new Date(data.deadline);
            document.getElementById('editDeadlineDate').value = deadline.toISOString().split('T')[0];
            document.getElementById('editDeadlineTime').value = deadline.toTimeString().slice(0,5);
            
            // Show current file
            if (data.attachmentPath) {
                document.getElementById('editCurrentFile').innerHTML = '<i class="fa-solid fa-paperclip"></i> Current: ' + data.attachmentPath;
            } else {
                document.getElementById('editCurrentFile').innerHTML = '<span style="color:#9ca3af;">No attachment</span>';
            }
            
            // Show warning if submissions exist
            const warningEl = document.getElementById('editFormWarning');
            if (data.submitCount > 0) {
                warningEl.innerHTML = '<i class="fa-solid fa-triangle-exclamation"></i> This assignment has ' + data.submitCount + ' submission(s). You can only extend the deadline, not shorten it.';
        ...
        document.getElementById('editTitle').addEventListener('input', function() {
            document.getElementById('editTitleCounter').textContent = this.value.length;
            editFormChanged = true;
        });
        document.getElementById('editDesc').addEventListener('input', function() {
            document.getElementById('editDescCounter').textContent = this.value.length;
            editFormChanged = true;
        });
        document.getElementById('editDeadlineDate').addEventListener('change', () => editFormChanged = true);
        document.getElementById('editDeadlineTime').addEventListener('change', () => editFormChanged = true);
        document.getElementById('editFile').addEventListener('change', () => editFormChanged = true);
        
        // ========== DELETE ASSIGNMENT ==========
        function confirmDelete(assignID, title) {
            document.getElementById('deleteAssignmentID').value = assignID;
            document.getElementById('deleteAssignTitle').textContent = title;
        ...
        document.addEventListener('DOMContentLoaded', function() {
            showToast(<?php echo json_encode(strip_tags($msg)); ?>, <?php echo json_encode($msgType); ?>);
        });
        <?php endif; ?>
    </script>
