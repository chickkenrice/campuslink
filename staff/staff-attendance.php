...
$sql = "(SELECT s.*, c.courseName, c.courseID, f.facilityName,
               ...
               (SELECT COUNT(*) FROM student st INNER JOIN program_course pc ON st.programID = pc.programID AND pc.courseID = s.courseID AND st.currentYear = pc.year AND st.currentSemester = pc.semester WHERE st.programID = s.programID AND (st.tutGroup = s.tutGroup OR s.tutGroup = 'Combined')) as totalStudents,
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && !isset($_POST['export_csv'])) {
    ...
    if (isset($_POST['manual_attendance'])) {
        ...
        // Clear output buffer to prevent prevent_back_button_cache() from corrupting JSON response
...
function getStudentList($db, $scheduleID, $date) {
    $stmtClass = $db->prepare("SELECT programID, tutGroup, courseID FROM class_schedule WHERE scheduleID = ?");
    ...
    $stmtPC = $db->prepare("SELECT year, semester FROM program_course WHERE programID = ? AND courseID = ? LIMIT 1");
    $stmtPC->bind_param("ss", $classInfo['programID'], $classInfo['courseID']);
    $stmtPC->execute();
    $courseYearSem = $stmtPC->get_result()->fetch_assoc();
    ...
    // Filter by year and semester so only students in the correct year/semester see this course
    if ($courseYearSem) {
        $sql .= " AND s.currentYear = ? AND s.currentSemester = ?";
    }
    $sql .= " ORDER BY status DESC, s.studentName ASC";

    $stmt = $db->prepare($sql);
    if ($courseYearSem) {
        $stmt->bind_param("issssii", $scheduleID, $date, $classInfo['programID'], $classInfo['tutGroup'], $classInfo['tutGroup'], $courseYearSem['year'], $courseYearSem['semester']);
    } else {
...
<body>
    <div class="app-bg">
        <div class="main-card">
            <?php $currentPage = 'attendance'; include __DIR__ . '/../includes/staffMaster.php'; ?>
            ...
            <main class="dashboard">
                ...
                ">
                    ...
                    <div style="position: relative; z-index: 1;">
                        <h1 style="font-size: 30px; font-weight: 700; margin: 0 0 8px 0; color: white;">Attendance Manager</h1>
                        <p style="margin: 0; font-size: 14px; color: rgba(255,255,255,0.85);"><?php echo date('l, d F Y'); ?></p>
                    </div>
                    
                    <!-- Right Decorative Illustration -->
                    <div style="position: relative; z-index: 1; display: flex; align-items: center; gap: 15px;">
                        <!-- Attendance List Clipboard -->
                        <div style="
                        ...
                        ">
                            <!-- Clipboard clip -->
                            <div style="position: absolute; top: -8px; left: 50%; transform: translateX(-50%); width: 30px; height: 12px; background: #6c5ce7; border-radius: 4px;"></div>
                            <!-- Checklist items -->
                            <div style="display: flex; flex-direction: column; gap: 8px; margin-top: 5px;">
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <div style="width: 14px; height: 14px; background: #10b981; border-radius: 4px; display: flex; align-items: center; justify-content: center;"><i class="fa-solid fa-check" style="color: white; font-size: 8px;"></i></div>
                                    <div style="height: 6px; background: #e5e7eb; border-radius: 3px; flex: 1;"></div>
                                </div>
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <div style="width: 14px; height: 14px; background: #10b981; border-radius: 4px; display: flex; align-items: center; justify-content: center;"><i class="fa-solid fa-check" style="color: white; font-size: 8px;"></i></div>
                                    <div style="height: 6px; background: #e5e7eb; border-radius: 3px; flex: 1;"></div>
                                ...
                                <div style="display: flex; align-items: center; gap: 6px;">
                                    <div style="width: 14px; height: 14px; border: 2px solid #d1d5db; border-radius: 4px;"></div>
                                    <div style="height: 6px; background: #e5e7eb; border-radius: 3px; flex: 1;"></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div style="
                            ...
                            position: relative;
                        ">
                            <i class="fa-solid fa-users" style="font-size: 28px; color: #8056ff;"></i>
                            <!-- Badge -->
    ...
    <script>
        ...
        function openModal(students, courseName, scheduleID, dateStr = '<?php echo $currentDate; ?>') {
            ...
            attendanceChanged = false; // Reset change tracking
            ...
            if (students.length === 0) container.innerHTML = '<p style="text-align:center;">No students.</p>';
                ...
                const row = `
                    <div class="student-row" data-student-id="${s.studentID}" style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #f3f4f6;">
        ...
        function toggleAttendance(studentID, status, btnElement) {
            ...
            fetch('staff-attendance.php', { method: 'POST', body: formData })
                .then(r => r.json())
                .then(d => {
                    if(d.status === 'success') {
                        // Update the UI without page reload
                        attendanceChanged = true;
                        updateStudentRow(studentID, status, btnElement);
                    } else {
        ...
        function updateStudentRow(studentID, newStatus, btnElement) {
            // Find the student row
            const row = btnElement.closest('.student-row');
            if (!row) return;

            // Get status badge and action container
            const statusContainer = row.querySelector('div[style*="text-align:right; display:flex; flex-direction:column"]');
            const actionContainer = row.querySelector('div[style*="text-align:right; display:flex; align-items:center"]');
            ...
            const timeDisplay = `<div style="font-size:11px; color:#6b7280; margin-top:3px; font-weight:500;"><i class="fa-regular fa-clock" style="font-size:10px; margin-right:4px;"></i>${displayHours}:${minutes} ${ampm}</div>`;
            const noTimeDisplay = `<div style="font-size:11px; color:#d1d5db; margin-top:3px;">-- : --</div>`;

            // Update based on new status
            if (newStatus === 'Present') {
                // Update badge to PRESENT
                statusContainer.innerHTML = `
                    <span style="background:#d1fae5; color:#065f46; padding:4px 10px; border-radius:12px; font-size:11px; font-weight:700;">PRESENT</span>
                    ${timeDisplay}
                `;
                // Update button to Undo
                btnElement.textContent = 'Undo';
                btnElement.style.background = 'transparent';
                btnElement.style.color = '#991b1b';
                btnElement.style.border = '1px solid #fee2e2';
                btnElement.style.padding = '4px 8px';
                btnElement.style.fontSize = '10px';
                btnElement.onclick = () => toggleAttendance(studentID, 'Absent', btnElement);
                btnElement.disabled = false;
            } else if (newStatus === 'Absent') {
                // Update badge to ABSENT
                statusContainer.innerHTML = `
                    <span style="background:#fee2e2; color:#991b1b; padding:4px 10px; border-radius:12px; font-size:11px; font-weight:700;">ABSENT</span>
                    ${noTimeDisplay}
                `;
                // Update button to Mark Present
                btnElement.textContent = 'Mark Present';
                btnElement.style.background = '#8056ff';
                btnElement.style.color = 'white';
                btnElement.style.border = 'none';
                btnElement.style.padding = '5px 10px';
                btnElement.style.fontSize = '11px';
                btnElement.onclick = () => toggleAttendance(studentID, 'Present', btnElement);
                btnElement.disabled = false;
            }

            // Update the modal header stats
            updateModalStats();
        }

        function updateModalStats() {
            // Count all students by status from DOM
            ...
            allRows.forEach(row => {
                const statusBadge = row.querySelector('span[style*="border-radius:12px"]');
                if (statusBadge) {
            ...
            // Update header
            const title = document.getElementById('modalTitle');
            const courseName = title.querySelector('div > span')?.textContent || 'Attendance List';
            
            let statsHTML = `<span style="color:#059669;">Present: ${presentCount}</span>`;
            if (lateCount > 0) statsHTML += ` • <span style="color:#d97706;">Late: ${lateCount}</span>`;
            statsHTML += ` • <span style="color:#dc2626;">Absent: ${absentCount}</span>`;
        ...
        function closeModal() {
            ...
            // Only reload if attendance was actually changed
            if (attendanceChanged) {
                location.reload();
            }
        }
        
        window.onclick = function(e) { if (e.target == document.getElementById('listModal')) closeModal(); }
    </script>
