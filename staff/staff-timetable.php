...
<div class="app-bg">
    <div class="main-card">
        ...
        <?php $currentPage = 'timetable'; include __DIR__ . '/../includes/staffMaster.php'; ?>
            ...
            <section class="welcome-card" style="display:flex; justify-content:space-between; align-items:center; margin-bottom: 28px;">
                <div class="welcome-details">
                    ...
                    <div class="welcome-meta">
                        <span class="welcome-badge"><?php echo htmlspecialchars($userID); ?></span>
                        <span class="welcome-meta-text"><?php echo htmlspecialchars($staffName); ?></span>
                        <span class="welcome-meta-sep">&middot;</span>
                        <span class="welcome-meta-sub"><i class="fa-solid fa-chalkboard-user"></i> Staff</span>
                    </div>
                </div>
                <a href="staff-timetable-report.php" target="_blank" style="
                    ...
                    <div id="replacement-form-container" style="display: none;">
                        ...
                        <div class="replacement-form-grid">
                            ...
                            <div class="rform-group">
                                ...
                                <input type="date" id="rr-newDate" min="<?php echo date('Y-m-d'); ?>">
                            </div>
                            <div class="rform-group">
...
<script>
    ...
    function displayTermInfo() {
        ...
        const startDate = (() => { const [ty,tm,td] = termData.startDate.split('-'); const wd = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][new Date(termData.startDate+'T00:00:00').getDay()]; return `${wd}, ${td}-${tm}-${ty}`; })();
        const endDate = (() => { const [ty,tm,td] = termData.endDate.split('-'); const wd = ['Sunday','Monday','Tuesday','Wednesday','Thursday','Friday','Saturday'][new Date(termData.endDate+'T00:00:00').getDay()]; return `${wd}, ${td}-${tm}-${ty}`; })();
    ...
    async function submitReplacementRequest() {
        ...
        // Validate replacement date is not in the past
        const today = new Date().toISOString().split('T')[0];
        if (newDate < today) {
            showReplacementMsg('Replacement date cannot be in the past.', 'error');
            return;
        }

        // Validate replacement duration matches original class duration
        const selectedOption = document.getElementById('rr-scheduleID').options[document.getElementById('rr-scheduleID').selectedIndex];
        if (selectedOption && selectedOption.dataset.startTime && selectedOption.dataset.endTime) {
            const origStart = selectedOption.dataset.startTime.substring(0,5);
            const origEnd = selectedOption.dataset.endTime.substring(0,5);
            const [osH, osM] = origStart.split(':').map(Number);
            const [oeH, oeM] = origEnd.split(':').map(Number);
            const originalDuration = (oeH * 60 + oeM) - (osH * 60 + osM);

            const [rsH, rsM] = startTime.split(':').map(Number);
            const [reH, reM] = endTime.split(':').map(Number);
            const replacementDuration = (reH * 60 + reM) - (rsH * 60 + rsM);

            if (replacementDuration !== originalDuration) {
                showReplacementMsg(`Replacement class duration (${replacementDuration} min) must match original class duration (${originalDuration} min).`, 'error');
                return;
            }
        }
