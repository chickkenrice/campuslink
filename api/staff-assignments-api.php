...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'toggleArchive') {
    $assignID = intval($_POST['assignmentID']);
    $isArchived = intval($_POST['isArchived']);
    
    $stmt = $db->prepare("UPDATE assignments SET isArchived = ? WHERE assignmentID = ?");
    $stmt->bind_param("ii", $isArchived, $assignID);
    
    if ($stmt->execute()) {
        echo json_encode(['status' => 'success', 'message' => $isArchived ? 'Assignment archived' : 'Assignment unarchived']);
    } else {
...
if (isset($_GET['assignmentID'])) {
    ...
    if (!empty($submissions)) {
        ...
        foreach ($submissions as $row) {
            ...
            $date = date('d-m-Y g:i A', strtotime($row['submittedAt']));
            ...
            foreach ($submissionFiles as $sf) {
                ...
                $sfDate = date('d-m-Y g:i A', strtotime($sf['uploadedAt']));
                $filesHtml .= '
