...
// =========================================================
function generateNextUserID($db) {
    $result = $db->query("SELECT userID FROM users ORDER BY userID DESC LIMIT 1");
    if ($result && $row = $result->fetch_assoc()) {
        $lastID = $row['userID'];
        if (preg_match('/U(\d+)/', $lastID, $matches)) {
...
function generateRoleID($db, $role) {
    if ($role === 'Student') {
        // Student IDs follow format: YYWPXXXXX (e.g., 25WP12345)
        $yearPrefix = date('y') . 'WP';
        // Get highest student ID with this year prefix
        $result = $db->query("SELECT studentID FROM student WHERE studentID LIKE '{$yearPrefix}%' ORDER BY studentID DESC LIMIT 1");
        if ($result && $row = $result->fetch_assoc()) {
            $lastID = $row['studentID'];
            $numPart = intval(substr($lastID, 4)) + 1;
            return $yearPrefix . str_pad($numPart, 5, '0', STR_PAD_LEFT);
        }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    elseif ($action === 'create') {
        ...
        $coursesList = $_POST['courses'] ?? []; // array from multi-select
        $currentYear = max(1, min(3, (int)($_POST['currentYear'] ?? 1)));
        $currentSemester = max(1, min(3, (int)($_POST['currentSemester'] ?? 1)));
        $tutGroup = $_POST['tutGroup'] ?? '';
        
        // Auto-generate IDs
        $userID = generateNextUserID($db);
        $roleID = generateRoleID($db, $role);
        
        if (!$userID || !$roleID) {
            $message = "Error generating user ID.";
        } else {
            // Duplicate email check across all role tables
            $dupEmail = false;
            if (!empty($email)) {
                $emailCheckSql = "SELECT email FROM student WHERE email = ? UNION SELECT email FROM staff WHERE email = ? UNION SELECT email FROM admin WHERE email = ?";
                $emailStmt = $db->prepare($emailCheckSql);
                $emailStmt->bind_param("sss", $email, $email, $email);
                $emailStmt->execute();
            ...
            // Duplicate phone check across all role tables
            $dupPhone = false;
            if (!empty($phone)) {
                $phoneCheckSql = "SELECT contactNo FROM student WHERE contactNo = ? UNION SELECT contactNo FROM staff WHERE contactNo = ? UNION SELECT contactNo FROM admin WHERE contactNo = ?";
                $phoneStmt = $db->prepare($phoneCheckSql);
                $phoneStmt->bind_param("sss", $phone, $phone, $phone);
                $phoneStmt->execute();
            ...
            if ($dupEmail) {
                $message = "Error: Email '$email' is already in use by another user.";
            } elseif ($dupPhone) {
                $message = "Error: Phone number '$phone' is already in use by another user.";
            } else {
            // 1. Insert into Parent Table (USERS)
            // Set default password as FirstName@123456 (hashed)
            $firstName = explode(' ', trim($name))[0];
            $defaultPassword = $firstName . '@123456';
            $hashedPassword = password_hash($defaultPassword, PASSWORD_DEFAULT);
            $stmt = $db->prepare("INSERT INTO users (userID, role, password) VALUES (?, ?, ?)");
            $stmt->bind_param("sss", $userID, $role, $hashedPassword);
            
            if ($stmt->execute()) {
                // 2. Insert into Child Table
                if ($role === 'Student') {
                    $stmt = $db->prepare("INSERT INTO student (studentID, userID, studentName, email, programID, currentYear, currentSemester, tutGroup) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssssiss", $roleID, $userID, $name, $email, $programme, $currentYear, $currentSemester, $tutGroup);
                } elseif ($role === 'Staff') {
                    $stmt = $db->prepare("INSERT INTO staff (staffID, userID, staffName, email, staffType) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssss", $roleID, $userID, $name, $email, $staffType);
                } elseif ($role === 'Admin') {
                    $stmt = $db->prepare("INSERT INTO admin (adminID, userID, adminName, email, contactNo) VALUES (?, ?, ?, ?, ?)");
                    $stmt->bind_param("sssss", $roleID, $userID, $name, $email, $phone);
                }

                if (isset($stmt)) {
                    try {
                        if ($stmt->execute()) {
                            // Save staff-course assignments
                            if ($role === 'Staff' && !empty($coursesList)) {
                                $scStmt = $db->prepare("INSERT IGNORE INTO staff_course (staffID, courseID) VALUES (?, ?)");
                                foreach ($coursesList as $cID) {
                                    $cID = trim($cID);
                                    if ($cID) {
                                        $scStmt->bind_param("ss", $roleID, $cID);
                                        $scStmt->execute();
                                    }
                                }
                            }
                            ...
                            // Log user creation
                            $logUserID = $_SESSION['actual_user_id'] ?? $_SESSION['user_id'];
                            logActivity($db, $logUserID, 'USER_CREATE', "Created new $role: $name (ID: $roleID)", [
                                'newUserID' => $userID,
                                'newRoleID' => $roleID,
                                'newUserRole' => $role,
                                'newUserName' => $name,
                            ...
                            $message = "New $role added successfully with ID: $roleID (Default password: {$firstName}@123456)";
                        } else {
    ...
    elseif ($action === 'update') {
        ...
        $dupEmail = false;
        if (!empty($email)) {
            $emailCheckSql = "SELECT email FROM student WHERE email = ? AND studentID != ? UNION SELECT email FROM staff WHERE email = ? AND staffID != ? UNION SELECT email FROM admin WHERE email = ? AND adminID != ?";
            $emailStmt = $db->prepare($emailCheckSql);
            $emailStmt->bind_param("ssssss", $email, $id, $email, $id, $email, $id);
            $emailStmt->execute();
            if ($emailStmt->get_result()->num_rows > 0) {
                $dupEmail = true;
            }
...
// For the 'all' tab, the displayed ID column is u.userID (not childID)
// so id_asc/id_desc must sort by u.userID instead of childID
$allOrderByClause = $orderByClause;
if ($sortBy === 'id_asc') $allOrderByClause = "u.userID ASC";
if ($sortBy === 'id_desc') $allOrderByClause = "u.userID DESC";
...
if ($activeTab === 'student') {
    ...
    $sql = "SELECT staffID as childID, userID, staffName as name, email, 'Staff' as role, staffType as extra, '' as grp,
            COALESCE(
              NULLIF((SELECT GROUP_CONCAT(DISTINCT courseID ORDER BY courseID SEPARATOR ', ') FROM staff_course WHERE staffID = staff.staffID), ''),
              (SELECT GROUP_CONCAT(DISTINCT courseID ORDER BY courseID SEPARATOR ', ') FROM class_schedule WHERE staffID = staff.staffID)
            ) as courses
    ...
    $sql = "SELECT u.userID, u.role,
            ...
            COALESCE(
              NULLIF((SELECT GROUP_CONCAT(DISTINCT courseID ORDER BY courseID SEPARATOR ', ') FROM staff_course WHERE staffID = st.staffID), ''),
              (SELECT GROUP_CONCAT(DISTINCT courseID ORDER BY courseID SEPARATOR ', ') FROM class_schedule WHERE staffID = st.staffID)
            ) as courses
            FROM users u
            LEFT JOIN student s ON u.userID = s.userID
            LEFT JOIN staff st ON u.userID = st.userID
            LEFT JOIN admin a ON u.userID = a.userID
            ORDER BY " . $allOrderByClause;
}
$result = $db->query($sql);
...
<body>
    ...
    <div class="app-bg">
        <div class="main-card">
            ...
            <?php $currentPage = 'users'; include __DIR__ . '/../includes/adminMaster.php'; ?>
            ...
            <main class="dashboard">
                ...
                <section class="announcements-card" style="margin-top: 20px;">
                        ...
                        <div id="addUserFormContainer" class="add-user-form-container">
                            ...
                            <form method="POST" onsubmit="return validateForm()" novalidate>
                                ...
                                <div class="form-grid-2">
                                    ...
                                    <!-- Year field for Students -->
                                    <div class="form-field" id="yearField" style="display: none;">
                                        <label for="currentYear">Study Year <span style="color: red;">*</span></label>
                                        <select id="currentYear" name="currentYear" onchange="clearFieldError(this)">
                                    ...
                                    <div class="form-field" id="semesterField" style="display: none;">
                                        <label for="currentSemester">Semester <span style="color: red;">*</span></label>
                                        <select id="currentSemester" name="currentSemester" onchange="clearFieldError(this)">
                                    ...
                                    <!-- Tut Group field for Students -->
                                    <div class="form-field" id="tutGroupField" style="display: none;">
                                        <label for="tutGroup">Tut Group <span style="color: red;">*</span></label>
                                        <select id="tutGroup" name="tutGroup" onchange="clearFieldError(this)">
                                    ...
                                    <div class="form-field" id="staffTypeField" style="display: none;">
                                        ...
                                        <select id="staffType" name="staffType" onchange="clearFieldError(this)">
                                            ...
                                            <option value="Lecturer">Lecturer &mdash; Lectures only</option>
                                            <option value="Tutor">Tutor &mdash; Practical &amp; Tutorial</option>
                                            <option value="Both">Both &mdash; Lecture, Practical &amp; Tutorial</option>
                                        </select>
                                    </div>
                                    
                                    <!-- Course field for Staff (multi-select with tags) -->
                                    <div class="form-field" id="courseField" style="display: none; grid-column: 1 / -1;">
                                        <label>Assigned Courses <span style="color: red;">*</span></label>
                                        <select id="coursePickerSelect" onchange="addCourse(this)">
                                            <option value="">— Select a course to add —</option>
                                            <?php 
                                            if ($courses && $courses->num_rows > 0) {
                                                while($crs = $courses->fetch_assoc()) {
                                                    echo '<option value="' . htmlspecialchars($crs['courseID']) . '" data-name="' . htmlspecialchars($crs['courseName']) . '">' . htmlspecialchars($crs['courseID']) . ' - ' . htmlspecialchars($crs['courseName']) . '</option>';
                                                }
                                            }
                                            ?>
                                        </select>
                                        <div id="selectedCourseTags" style="display:flex; flex-wrap:wrap; gap:6px; margin-top:8px; min-height:36px; padding:8px 10px; background:#f9f7ff; border:1px dashed #c8b8ff; border-radius:8px; align-items:center;">
                                            <span id="courseTagPlaceholder" style="color:#bbb; font-size:13px;">No courses selected yet — pick from the dropdown above</span>
                                        </div>
                                        <div id="courseError" style="color:#ff5f73; font-size:12px; font-weight:600; margin-top:4px; display:none;"><i class="fa-solid fa-exclamation-circle"></i> At least one course must be assigned</div>
                                ...
                                <div style="background: #f0f7ff; padding: 12px; border-radius: 8px; margin-top: 16px; font-size: 13px; color: #0366d6;">
                                    ...
                                    <strong>Note:</strong> User ID will be auto-generated. Default password is <strong>FirstName@123456</strong> (e.g. Arvind@123456) - users should change it after first login.
                                </div>
                    ...
                    <div class="tab-header-container">
                        <nav class="tab-nav">
                            <a href="?tab=all&sort=<?php echo htmlspecialchars($sortBy); ?>" class="tab-btn <?php echo $activeTab == 'all' ? 'active' : ''; ?>">All Users</a>
                            <a href="?tab=student&sort=<?php echo htmlspecialchars($sortBy); ?>" class="tab-btn <?php echo $activeTab == 'student' ? 'active' : ''; ?>">Students</a>
                            <a href="?tab=staff&sort=<?php echo htmlspecialchars($sortBy); ?>" class="tab-btn <?php echo $activeTab == 'staff' ? 'active' : ''; ?>">Staff</a>
                            <a href="?tab=admin&sort=<?php echo htmlspecialchars($sortBy); ?>" class="tab-btn <?php echo $activeTab == 'admin' ? 'active' : ''; ?>">Admins</a>
                        </nav>
                    ...
                    <div style="margin-bottom: 12px;">
                        <div style="position: relative; max-width: 400px;">
                            <i class="fa-solid fa-magnifying-glass" style="position:absolute; left:12px; top:50%; transform:translateY(-50%); color:#9ca3af; font-size:14px;"></i>
                            <input type="text" id="userSearchInput" placeholder="Search by name, ID or email..." oninput="filterUserTable()" style="width:100%; padding:9px 12px 9px 36px; border:1px solid #e0e0e0; border-radius:8px; font-size:14px; box-sizing:border-box; outline:none;" onfocus="this.style.borderColor='#8056ff'" onblur="this.style.borderColor='#e0e0e0'">
                        </div>
                    </div>
    ...
    <script>
        ...
        function filterUserTable() {
            ...
            const rows = document.querySelectorAll('.student-table tbody tr[style*="border-bottom"]');
            rows.forEach(row => {
                const id   = row.querySelector('.id-cell')?.textContent.toLowerCase() || '';
                const name = row.querySelector('.name-cell')?.textContent.toLowerCase() || '';
                const email = row.cells[3]?.textContent.toLowerCase() || '';
                row.style.display = (!q || id.includes(q) || name.includes(q) || email.includes(q)) ? '' : 'none';
            });
        }
        ...
        function toggleRoleFields() {
            ...
            const tutGroupField = document.getElementById('tutGroupField');
            ...
            if (!role) {
                ...
                tutGroupField.style.display = 'none';
            ...
            if (role === 'Student') {
                ...
                tutGroupField.style.display = 'block';
                ...
                resetCourseTags(); // clear any previously selected courses
            }
        }
        ...
        function validateForm() {
            ...
            if (role === 'Student') {
                ...
                const yr = document.getElementById('currentYear').value;
                const sem = document.getElementById('currentSemester').value;
                ...
                if (!yr) {
                    showError('currentYear', 'Study year is required for students');
                    return false;
                }
                if (!sem) {
                    showError('currentSemester', 'Semester is required for students');
                    return false;
                }
                const grp = document.getElementById('tutGroup').value;
                if (!grp) {
                    showError('tutGroup', 'Tut group is required for students');
                    return false;
                }
            } else if (role === 'Staff') {
                const staffType = document.getElementById('staffType').value.trim();
                const courseCount = parseInt(document.getElementById('courseCount').value || '0');
                ...
                if (courseCount === 0) {
                    ...
                    document.getElementById('courseField').scrollIntoView({ behavior: 'smooth', block: 'center' });
                    return false;
                }
            } else if (role === 'Admin') {
        ...
        // ── Multi-select course tag functions (for Staff creation) ──
        const selectedCourses = {};

        function addCourse(selectEl) {
            const courseID = selectEl.value;
            if (!courseID) return;
            if (selectedCourses[courseID]) { selectEl.value = ''; return; } // already added
            selectedCourses[courseID] = selectEl.options[selectEl.selectedIndex].getAttribute('data-name') || courseID;
            document.getElementById('courseCount').value = Object.keys(selectedCourses).length;
        ...
        function renderCourseTags() {
            const container = document.getElementById('selectedCourseTags');
            const placeholder = document.getElementById('courseTagPlaceholder');
            container.querySelectorAll('.course-chip').forEach(c => c.remove());
            document.querySelectorAll('input[name="courses[]"]').forEach(i => i.remove());
            ...
            if (ids.length === 0) {
                ...
                ids.forEach(id => {
                    ...
                    chip.style.cssText = 'display:inline-flex;align-items:center;gap:5px;background:#ede9ff;color:#6d28d9;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;';
                    chip.innerHTML = id + ' <span title="' + selectedCourses[id] + '" style="font-weight:400;color:#9061f9;font-size:11px;max-width:120px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;">(' + selectedCourses[id] + ')</span>' +
                        '<button type="button" onclick="removeCourse(\'' + id + '\')" style="background:none;border:none;color:#9061f9;cursor:pointer;font-size:15px;padding:0;line-height:1;margin-left:3px;">&times;</button>';
